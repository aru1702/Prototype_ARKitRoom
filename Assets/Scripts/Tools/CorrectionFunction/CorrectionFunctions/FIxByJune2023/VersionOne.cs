...
namespace CorrectionFunctions
{
    ...
    public class VersionOne : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (m_MathFunction == "") m_MathFunction = MathFunctions.SIGMOID;
        ...
        void Main()
        {
            ...
            List<float[]> weights = !m_AddThreshold ?
                OTM.GetAllWeights(MathFunctions.SIGMOID, true, true, m_ScalarWeight) :
                OTM.GetAllWeightsThreshold(MathFunctions.SIGMOID, true, true, m_ScalarWeight, m_Threshold);
            ...
            for (int i = 0; i < m_Objects.Count; i++)
            {
                ...
                // calculate new object rotation with weight
                if (m_AlsoWithRotatingObject)
                {
                    List<EigenMacHelper.QuaternionWeighted> qws = new();
                    for (int j = 0; j < m_Markers.Count; j++)
                    {
                        Quaternion diff = Quaternion.Inverse(m_Markers[j].GT_Rotation)
                                                           * m_Markers[j].C_Rotation;
                    ...
                    m_Objects[i].transform.rotation = m_InitObjectsRotation[i] * new_rot;
                }
            }
        }
        ...
        bool m_AlsoWithRotatingObject = false;
        public void AlsoWithRotatingObject(bool t) { m_AlsoWithRotatingObject = t; }

        public void SetMathFunction(string s) { m_MathFunction = s; }

        bool m_AddThreshold = false;
        float m_Threshold = 0;
        public void AddThreshold(bool t) { m_AddThreshold = t; }
        public void Threshold(float f) { m_Threshold = f; }
    }
}
